	...
	while(1)// repeat forever:
	{
		...
		for(i = 0; i < 128; i++) {
			for(j = 0; j < 64; j++) {
				...
				if(i!=0 || j!=0) d[pos] += -(image[pos-128-1] << 1); 	    // Canto Superior Esquerdo
				if(i!=128 || j!=64) d[pos] += -(image[pos+128+1] << 1); 	// Canto Inferior Direito
				if(j!=0) d[pos] += -(image[pos-128] << 1); 				    //Linha de Cima
				if(j!=64) d[pos] += -(image[pos+128] << 1);				    //Linha de Baixo
				if(i!=0) d[pos] += -(image[pos-1] << 1); 				  	//Coluna Esquerda
				if(i!=128) d[pos] += -(image[pos+1] << 1);				    //Coluna Direita
